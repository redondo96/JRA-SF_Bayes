...
pairs_posterior_2 <- function(posterior, strip_text = 3, axis_text_size = 4) {

  if("I0" %in% colnames(posterior)) {
    posterior <- rename(posterior, `I(0)` = I0)
  }

  if("E0" %in% colnames(posterior)) {
    posterior <- rename(posterior, `E(0)` = E0)
  }

  # if("gamma_1" %in% colnames(posterior)) {
    # posterior <- rename(posterior, bquote(gamma[1]) = gamma_1)
  # }
...
prior_posterior_plot_3 <- function(pars_df) {
  beta_df <- dplyr::select(pars_df, beta)
  ...
  gamma_1_df <- dplyr::select(pars_df, gamma_1)

  base <- ggplot(gamma_1_df, aes(x = gamma_1)) +
    geom_density(colour = "steelblue") +
    scale_x_continuous(limits = c(0, 1))
  ...
  # marrangeGrob(g1, g2, g3, g4, g5, g6, g7, nrow = 3, ncol = 3)
}

trace_plot_2 <- function(fit, pars, n_samples = 2000, n_warmup = 1000, subtitle = "") {
